...
<div class="inline-block relative {width} lg:{width} mr-2 lg:mr-0" use:clickOutside on:click_outside={() => {isVisible = false}}>
    {#if screenWidth > 1280}
        {#if kind === 'markdown'}
            <div on:click={() => {isVisible = !isVisible}} class="grid grid-flow-col cursor-pointer content-center relative h-11 outline-none bg-purple-100 rounded-lg px-4" aria-hidden="true">
